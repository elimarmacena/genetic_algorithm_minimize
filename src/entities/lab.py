...
class Lab:
    ...
    """
    def championship(self,total_pick:int = 2):
        current_round = 0
        winner_list = list()
        while current_round < self.population_size:
            ...
            round_winner = randint(0,total_pick-1)
    ...
    """
        The crossover work with 60% as default, when the necessary rate isn't achived
        are used the two parents as result. This way we can have subjects without 
        crossover as final result.
        @Return: list of subject(class)
    """
    def crossover(self,cross_rate:float=0.6):
        cross_populaltion = []
    ...
    """
        ...
        @Return: list of subject(class)
    """
    def mutation(self,mutation_rate:float=0.01):
        mutation_population = []
    ...
    """
        ...
        @Return: list of subject(class)
    """
    def create_new_gen(self,elite_percent:float=0.1):
        ...
        new_gen = list()
        new_gen = self.current_population[:int(self.population_size * elite_percent)]
        new_gen = new_gen + self.mutated_population[:-int(self.population_size * elite_percent)]
